...
const orderQueueStyles = sva({
  slots: ["container", "table", "headerRow", "headerCell", "bodyRow", "bodyCell", "receiptNumber", "itemDetails", "orderItem", "itemNumber", "itemName", "itemPrice", "itemAmount"],
  base: {
    container: {
      h: "full",
      maxH: "full",
...
const DEFAULT_VISIBLE_COLUMNS: Required<NonNullable<OrderQueueProps["visibleColumns"]>> = {
  receiptNumber: true,
  itemNumber: true,
  itemContent: true,
  status: true,
...
const ConfirmedOrdersList = memo(({ size = "small", filterGroupId, filterStatus, visibleColumns }: { size?: "small" | "large"; filterGroupId?: string; filterStatus?: Order["status"]; visibleColumns?: OrderQueueProps["visibleColumns"] }): ReactElement => {
  ...
  const ordersWithItemNumbers = useMemo<DisplayOrder[]>(() => {
    ...
    if (filterGroupId != null) {
      filteredOrders = filteredOrders.filter((order) =>
        order.items.some((item) => itemGroupMap.get(item.id) === filterGroupId),
      );
    }
...
const UnconfirmedOrderPreview = memo(({ size = "small", filterGroupId, visibleColumns }: { size?: "small" | "large"; filterGroupId?: string; visibleColumns?: OrderQueueProps["visibleColumns"] }): ReactElement | null => {
  ...
  if (filterGroupId != null) {
    ...
    const hasTargetGroup = normalizedItems.some((item) => itemGroupMap.get(item.id) === filterGroupId);
    if (!hasTargetGroup) {
      return null;
    }
  }
...
export function OrderQueue({ size = "small", filterGroupId, filterStatus, visibleColumns = DEFAULT_VISIBLE_COLUMNS }: OrderQueueProps = {}): ReactElement {
  const orders = useStore($orders);
  const normalizedItems = useStore($normalizedCurrentOrderItems);
            ...
            {size === "large"
              ? (
                  <p.tr className={styles.headerRow}>
                    ...
                    {visibleColumns.receiptNumber !== false && (
                      <p.th className={styles.headerCell} w="32">受付番号</p.th>
                    )}
                    {visibleColumns.itemContent !== false && (
                      <p.th className={styles.headerCell} w="stretch">
                        ...
                        >
                          ...
                          <p.div textAlign="right" w="40">価格 [円]</p.div>
                          <p.div w="24">数量</p.div>
                        </p.div>
                      </p.th>
                    )}
                    {visibleColumns.status !== false && (
                      <p.th className={styles.headerCell} w="48">状態</p.th>
                    )}
                    {visibleColumns.orderTime !== false && (
                      <p.th className={styles.headerCell} w="40">注文時刻</p.th>
                    )}
                    {visibleColumns.pickupTime !== false && (
                      <p.th className={styles.headerCell} w="40">受取時刻</p.th>
                    )}
                  </p.tr>
                )
              : (
                  <p.tr className={styles.headerRow}>
                    ...
                    {visibleColumns.receiptNumber !== false && (
                      <p.th className={styles.headerCell} w="20">受付番号</p.th>
                    )}
                    {visibleColumns.itemContent !== false && (
                      <p.th className={styles.headerCell} w="stretch">
                        注文内容
                    ...
                    {visibleColumns.status !== false && (
                      <p.th className={styles.headerCell} w="24">状態</p.th>
                    )}
                    {visibleColumns.orderTime !== false && (
                      <p.th className={styles.headerCell} w="20">注文時刻</p.th>
                    )}
                    {visibleColumns.pickupTime !== false && (
                      <p.th className={styles.headerCell} w="20">受取時刻</p.th>
                    )}
                  </p.tr>
                )}
          </Table.head>
